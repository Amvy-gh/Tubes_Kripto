...
class SteganoGUI(QMainWindow):
    def __init__(self):
        ...
        # Encrypt button
        encrypt_btn = QPushButton("Encrypt and Embed")
        encrypt_btn.clicked.connect(self.encrypt_process)
    ...
    def encrypt_process(self):
        try:
            ...
            self.encrypt_status.setText("Generating RSA keys...")
            ...
            # Encrypt text
            self.encrypt_status.setText("Encrypting text...")
            encrypted_data = encrypt_data(public_key, text)
            if len(encrypted_data) > 256:
                raise ValueError("Text too long for RSA encryption!")
            self.encrypt_progress.setValue(50)
            
            # Create QR
            self.encrypt_status.setText("Generating QR code...")
            ...
            # Embed in audio
            self.encrypt_status.setText("Embedding data in audio...")
            stego_file = embed_data_in_audio(self.audio_path, compressed_data)
            self.encrypt_progress.setValue(100)
            
            self.encrypt_status.setText(f"Success! Saved as: {stego_file}")
            QMessageBox.information(self, "Success", 
                                 "Encryption complete!\n\n"
                                 f"Stego audio saved as: {stego_file}\n"
    ...
    def decrypt_process(self):
        try:
            if not self.stego_path:
                raise ValueError("Please select a stego audio file!")
            if not self.private_key_path:
                raise ValueError("Please select a private key file!")
            
            self.decrypt_status.setText("Loading private key...")
            self.decrypt_progress.setValue(10)
            
            # Load private key
            private_key = load_private_key(self.private_key_path)
            self.decrypt_progress.setValue(30)
            
            # Extract data
            self.decrypt_status.setText("Extracting hidden data...")
            extracted_data = extract_data_from_audio(self.stego_path, float('inf'))
            self.decrypt_progress.setValue(60)
            
            # Decrypt
            self.decrypt_status.setText("Decrypting data...")
